...
Skip agent hook installation and system prompt injection. When set, nono will not install the `PostToolUseFailure` hook into `~/.claude/hooks/` and will not pass `--append-system-prompt-file` to Claude Code.

This is useful when the hook produces false positives (firing on non-sandbox errors) or when you want to debug agent behavior without nono's injected context.

```bash
nono run --no-hooks --profile claude-code --allow-cwd -- claude
```

| Environment Variable | `NONO_NO_HOOKS` |
|---------------------|-----------------|
| Example | `NONO_NO_HOOKS=true` |
...
| `--no-hooks` | `NONO_NO_HOOKS` | `NONO_NO_HOOKS=true` |
